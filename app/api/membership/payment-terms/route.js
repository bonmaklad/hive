...
function safeText(value, limit = 32) {
...
export async function POST(request) {
    ...
    const paymentTerms = safeText(payload?.payment_terms, 32);
    if (!['invoice', 'auto_card'].includes(paymentTerms)) {
        return NextResponse.json({ error: 'payment_terms must be invoice or auto_card.' }, { status: 400 });
    }
    ...
    const { data: updated, error: updateError } = await admin
        .from('memberships')
        .update({ payment_terms: paymentTerms, updated_at: new Date().toISOString() })
        .eq('id', membership.id)
        .select('*')
        .single();
